...
- **Проект** - сущность, которая имеет ассоциированный с ней канал-форум, роли и таски. Проекты собирают статистику по всем пользователям, которые имеют роль, ассоциированную с проектом.
  ...
  **Параметр проекта**:
  - **максимальное количество тасков на человек** - во скольком количестве тасков максимально может участвовать любой польователь.
  - **роль ждуна** (не обязательно) - роль, которую будет пинговать при каждом создании тасков проекта.
  - **форум** - канал-форум для тасков проекта, все публикации в этом канале станут интерпретироватся как таски.
  - **основной канал** - основной канал проекта, на данный момент этот параметр ни на что не влияет.
  - **канал статистики** - в данном канал будер размещён автообновляемый пост статистики проекта.
  - **роли проректа** - discord роли, ассоциированные с данним проектом, все кто имеют любую из этим ролей смогут вступать в таски и будут отображатся в статистике.
...
- **Таски** - публикации в канале-форуме любого проекта, таск может иметь сложность (базовые очки за его выполенние) и максимальное количество человек, которое может в него вступить.
  Пользователи, которые имеют роль, асоциированную с проектом могут вступать в любые таски проекта, пока не достигнут лимита по таскам на одного человека, который настраивается в конфиге проекта.
  ...
  **Параметры тасков** (все данные параметры определяются автоматически при создании публикации или её обновлении):
  ...
  - **дата создания**
  - **базовые очки за выполение таска** (определяется тегами)
  - **последний сейв** - параметр, который испульзуется для маппинг тасков, его можно задавать командой /последний-сейв
  - **бригадир** - куратор данного таска, кто ответственнен за координацию работы его участников, задаётся командой /взять-заказ
  - **члены бригады** - пополняются при вступлении в таск
  Чтобы вступить в таск нужно нажать на базовое эмодзи форум-канала тасков. Для завершения таска нужно испольовать команду /закрыть, по завершении члены таска получат очки за работу, количество очков определяет тот, кот закрывает таск.
...
- **/сейв** - использует авторизацию по папке и даёт скачать только из личной папки по отностильному пути.
- **/сейв-плюс** - даёт возможность скачать любой сейв по абсолютному пути.


**Авторизация по папке**

Чтобы пользователь мог скачивать сейвы ему нужно привязать свою папку с помощью **/привязать-папку**, команда имеет защиту от привязывания чужих папок, если будет такая попытка, то бот создат оповещение в ветке с оповещениями, если она настроена.


### Статистика пользователя
Любой пользователь имеет свою личную статистику, а именно:
- **в бригадах** - все таски, в которых состоит пользователь
- **выполенные заказы** - все таски, в выполнении которых пользователь участвовал.
...
- **последняя активность** - когда пользователь последний раз участвовал в выполеннии заказов.
...
Такие параметры как **в бригадах**, **выполенные заказы**, **курирование заказов** и **последняя активность** хранят информацию по каждому проекту, в котором участвует пользователь.
...
- добавить его на сервер с флагами `bot` и `aplications.command` в OAuth2 URL Generator, а также с правами `Administrator` в `Bot Permissions`.
...
## Создание конфига
В этом разделе подробно разберём как должна выглядеть папка **data** и как настроивать конфиг бота.

### Секретная конфигурация
Часть конфига бота, которую опасно выкладывать в полный доступ хранится в `data/.env`.
...
# название локализации    = коффициент, на который умножаются базовые очки таска
...
[Task]
brigadire_score_modifier  = 2.0 # коффициент, на который умножаются базовые очки таска для бригадира
```
...
- type: page # только прототипы с таким типом используются в магазине
  ...
  access: [+] # массив ролей, имея хотябы одну из которым пользователь сможет видеть данную страницу. Данное поле не обязательное, если не указать, то все будут иметь доступ к странице. Регистр названия роли не важен
  notAccess: [маппер] # массив ролей, имея хотябы одну из которым пользователь не сможет увидеть страницу, имеет больший приоритет чем поле acces. Данное поле необязательное. Регистр названия роли не важен
