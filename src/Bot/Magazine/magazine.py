...
class Magazine:
    ...
    def __init__(self, current_page, inter: disnake.CommandInteraction):
        self.pages = [
            ...
            Page(
                name= "Повышение",
                description= "Устали быть стажером? Хотите получить доступ к мастерской? Тогда вам к нам! Всего за 25 очков вы можете получить своё долгожданное повышение.",
                price= 18,
                on_buy= [give_role, rem_role],
                access= ["+"],
                not_access= ["маппер"],
                add_roles= ["маппер"],
            ...
            Page(
                ...
                on_buy= [send],
                log_channel= True,
                text= "<@1046425922200420505> надо раскошелиться на 700 рублей!"
            ),
            Page(
                ...
                on_buy= [send],
                log_channel= True,
                text= "<@1046425922200420505> надо раскошелиться на 1000 рублей!"
            )
        ]
...
async def give_role(inter: disnake.AppCommandInteraction, kwargs):
    for i in kwargs["add_roles"]:
        for role in inter.guild.roles:
            if role.name == i: await inter.author.add_roles(i)
...
async def send(inter: disnake.AppCommandInteraction, kwargs):
    if "log_channel" in kwargs and kwargs["log_channel"] == True:
        Logger.high(inter, kwargs["text"])
